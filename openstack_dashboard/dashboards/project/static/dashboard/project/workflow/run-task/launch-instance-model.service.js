...
(function () {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.project.workflow.run-task
   *
   * @description
  ...
  function RunTaskModel(
    ...
    function initialize(deep) {
      ...
      if (model.initializing) {
        ...
        var runTaskDefaults = settings.getSetting('LAUNCH_INSTANCE_DEFAULTS');
        ...
        promise = $q.all([
          ...
          runTaskDefaults.then(addImageSourcesIfEnabled, noop),
          runTaskDefaults.then(setDefaultValues, noop),
          runTaskDefaults.then(addVolumeSourcesIfEnabled, noop)
        ]);
    ...
    /**
     * @ngdoc method
     * @name RunTaskModel.createInstance
     * @returns {promise}
     *
     * @description
