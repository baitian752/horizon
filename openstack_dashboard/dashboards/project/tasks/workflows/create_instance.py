...
class SetInstanceDetailsAction(workflows.Action):
    ...
    class Meta(object):
        name = _("Details")
        help_text_template = ("project/tasks/"
    ...
    def _check_quotas(self, cleaned_data):
        ...
        # Prevent launching more tasks than the quota allows
        usages = quotas.tenant_quota_usages(
            self.request,
            targets=('tasks', 'cores', 'ram', 'volumes', ))
        available_count = usages['tasks']['available']
    ...
    def _check_source_volume(self, cleaned_data):
        ...
        # Prevent launching multiple tasks with the same volume.
        # TODO(gabriel): is it safe to launch multiple tasks with
        # a snapshot since it should be cloned to new volumes?
        count = cleaned_data.get('count', 1)
        if count > 1:
            msg = _('Launching multiple tasks is only supported for '
...
class SetNetwork(workflows.Step):
    ...
    template_name = "project/tasks/_update_networks.html"
...
class SetAdvancedAction(workflows.Action):
    ...
    def __init__(self, request, context, *args, **kwargs):
        ...
        try:
            ...
            # is supported.
            if context.get('workflow_slug') != 'run_task' or (
...
class LaunchInstance(workflows.Workflow):
    ...
    success_url = "horizon:project:tasks:index"
    multipart = True
    ...
    def format_status_message(self, message):
        ...
        if int(count) > 1:
            return message % {"count": _("%s tasks") % count,
                              "name": name}
        else:
