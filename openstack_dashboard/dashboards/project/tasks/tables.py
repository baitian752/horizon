...
from openstack_dashboard.dashboards.project.tasks.workflows \
...
class DeleteInstance(policy.PolicyTargetMixin, tables.DeleteAction):
    ...
    help_text = _("Deleted tasks are not recoverable.")
...
class RebootInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    help_text = _("Restarted tasks will lose any data"
...
class RescueInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:project:tasks:rescue"
...
class LaunchLink(tables.LinkAction):
    name = "launch"
    verbose_name = _("Run Task")
    url = "horizon:project:tasks:launch"
...
class LaunchLinkNG(LaunchLink):
    name = "launch-ng"
    url = "horizon:project:tasks:index"
    ajax = False
    classes = ("btn-launch", )

    def get_default_attrs(self):
        url = urls.reverse(self.url)
        ngclick = "modal.openRunTaskWizard(" \
            "{ successUrl: '%s' })" % url
        self.attrs.update({
            'ng-controller': 'RunTaskModalController as modal',
            'ng-click': ngclick
        })
...
class EditInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:project:tasks:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class EditPortSecurityGroups(tables.LinkAction):
    ...
    url = "horizon:project:tasks:detail"
    icon = "pencil"
...
class ResizeLink(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:project:tasks:resize"
...
class RebuildInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:project:tasks:rebuild"
...
class DecryptInstancePassword(tables.LinkAction):
    ...
    url = "horizon:project:tasks:decryptpassword"
...
class DisassociateIP(tables.LinkAction):
    ...
    url = "horizon:project:tasks:disassociate"
...
class LockInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    # to only allow unlocked tasks to be locked
    def allowed(self, request, instance):
...
class UnlockInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    # to only allow locked tasks to be unlocked
    def allowed(self, request, instance):
...
class AttachVolume(tables.LinkAction):
    ...
    url = "horizon:project:tasks:attach_volume"
    classes = ("ajax-modal",)
    policy_rules = (
...
class DetachVolume(AttachVolume):
    ...
    url = "horizon:project:tasks:detach_volume"
    policy_rules = (
...
class AttachInterface(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:project:tasks:attach_interface"
...
class DetachInterface(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    classes = ("btn-confirm", "ajax-modal")
    url = "horizon:project:tasks:detach_interface"
...
def get_ips(instance):
    template_name = 'project/tasks/_instance_ips.html'
    ip_groups = {}
...
def get_flavor(instance):
    if hasattr(instance, "full_flavor"):
        template_name = 'project/tasks/_instance_flavor.html'
...
def get_server_detail_link(obj, request):
    return get_url_with_pagination(request,
                                   ...
                                   'horizon:project:tasks:detail',
                                   obj.id)

def getTaskName(instance):
    return instance.metadata["task_name"]

def writeData(x):
    with open("/opt/stack/Desktop/test.type", "a") as f:
        f.write(str(type(x)) + "\n")
        f.write(str(x) + "\n")
        f.write(str(hasattr(x, "task_name")) + "\n")
        f.write(x.get("task_name", "None") + "\n")
